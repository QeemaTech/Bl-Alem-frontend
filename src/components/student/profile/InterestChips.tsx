...
export function InterestChips({ value, onChange, error }: InterestChipsProps) {
  ...
  const { suggestedInterests } = useStudentProfileLabels();
  ...
  return (
    <div className="student-interest-field">
      <label className="field" htmlFor="student-interests-input">
        <span>{t('student.interests.title')}</span>
      </label>

      {value.length ? (
        <ul className="student-interest-chips" aria-label={t('student.interests.selectedAria')}>
          {value.map((interest) => (
            <li key={interest}>
              <span className="student-interest-chip">
                ...
                <button
                  ...
                  aria-label={t('student.interests.removeAria', { interest })}
                >
      ...
      <form className="student-interest-add" onSubmit={handleSubmit}>
        <input
          ...
          placeholder={t('student.interests.placeholder')}
        ...
        <Button
          ...
          disabled={!draft.trim()}
        >
          {t('student.interests.add')}
        </Button>
      </form>

      {availableSuggestions.length ? (
        <div className="student-interest-suggestions">
          <span className="student-interest-suggestions-label">{t('student.interests.suggestions')}</span>
